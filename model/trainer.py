...
class Trainer:
    ...
    def __init__(
        ...
        llava_dataset: Optional[Dataset] = None,
        ...
        self.llava_dataset = llava_dataset
    ...
    def _get_llava_sampler(self) -> Optional[torch.utils.data.Sampler]:
        if self.llava_dataset is None or not has_length(self.llava_dataset):
            return None

        # Build the sampler.
        if self.args.group_by_length:
            if is_datasets_available() and isinstance(self.llava_dataset, datasets.Dataset):
                lengths = (
                    self.llava_dataset[self.args.length_column_name]
                    if self.args.length_column_name in self.llava_dataset.column_names
                    else None
                )
            else:
                lengths = None
            ...
            return LengthGroupedSampler(
                self.args.train_batch_size * self.args.gradient_accumulation_steps,
                dataset=self.llava_dataset,
                lengths=lengths,
                model_input_name=model_input_name,
            )

        else:
            return RandomSampler(self.llava_dataset)

    def get_llava_dataloader(self) -> DataLoader:

        llava_dataset = self.llava_dataset
        data_collator = self.data_collator
        if is_datasets_available() and isinstance(llava_dataset, datasets.Dataset):
            llava_dataset = self._remove_unused_columns(llava_dataset, description="training")
        else:
            data_collator = self._get_collator_with_removed_columns(data_collator, description="training")
        ...
        if not isinstance(llava_dataset, torch.utils.data.IterableDataset):
            dataloader_params["sampler"] = self._get_llava_sampler()
            dataloader_params["drop_last"] = self.args.dataloader_drop_last
            dataloader_params["worker_init_fn"] = seed_worker

        return self.accelerator.prepare(DataLoader(llava_dataset, **dataloader_params))
    ...
    def _inner_training_loop(
        ...
        if self.llava_dataset is not None:
            llava_dataloader = self.get_llava_dataloader()
            llava_iterator = iter(llava_dataloader)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if random.random() < 0.1:
                    try:
                        llava_inputs = next(llava_iterator)
                    except StopIteration:
                        llava_iterator = iter(llava_dataloader)
                        llava_inputs = next(llava_iterator)
                    llava_loss_step = self.training_step(model, llava_inputs)
                    tr_loss_step = tr_loss_step * 0.3 + llava_loss_step * 0.7
